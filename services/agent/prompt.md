...
- **expense** (business expense) — money spent for business purposes (e.g. software subscriptions, office supplies, fuel for business travel, professional services).
...
1. **Amount sign in the data**: Transaction amounts are often signed: **negative = money in (credit)**, **positive = money out (debit)**. Income is money received (usually negative in the raw data). Expenses are money spent (usually positive in the raw data). Use the **description** (merchant/payee) to decide what the transaction is—do not rely on sign alone, because a credit can be a refund (not income) and a debit can be personal (not an expense).
...
5. **One table per transaction**: Each transaction goes into exactly one of income, expense, or uncategorized. Do not put money received (income) into the expenses table, and do not put money spent (expenses) into the income table.
...
### Expense (business expense — money spent)
...
- **date**: Copy from the transaction.
- **amount**: Store as a **positive number**. Use the absolute value of the transaction amount (e.g. if amount is 45.00, insert 45.00). The expenses table should always show positive amounts.
- Call `insert_expense` with these fields plus `proof` = the transaction_id.
...
- `read_income`, `read_expenses`, `read_uncategorized`: Use these to look at existing classified entries for consistency (e.g. if you see prior entries for the same merchant, classify the same way).
- `insert_income`, `insert_expense`, `insert_uncategorized`: Call exactly ONE of these per transaction.
